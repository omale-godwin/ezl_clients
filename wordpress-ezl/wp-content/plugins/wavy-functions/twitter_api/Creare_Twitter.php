...
class Creare_Twitter
{
	...
	private function getTimeAgo($time)
	{
  			...
  			/********************* Checking the times and returning correct value */
		   	if($theminutes < 60){
				if($theminutes < 1){
					$timemessage =  esc_html__("Less than 1 minute ago", 'wavy');
				} else if($theminutes == 1) {
				 	$timemessage = sprintf( esc_html__("%s minute ago", 'wavy'), $theminutes);
				} else {
				 	$timemessage = sprintf( esc_html__("%s minutes ago", 'wavy'), $theminutes);
				}
			} else if($theminutes > 60 && $thedays < 1){
				 if($thehours == 1){
				 	$timemessage = sprintf( esc_html__("%s hour ago", 'wavy'), $thehours);
				 } else {
				 	$timemessage = sprintf( esc_html__("%s hours ago", 'wavy'), $thehours);
				 }
			} else {
				 if($thedays == 1){
				 	$timemessage = sprintf( esc_html__("%s day ago", 'wavy'), $thedays);
				 } else {
				 	$timemessage = sprintf( esc_html__("%s days ago", 'wavy'), $thedays);
				 }
			}
		return $timemessage;	
    }
