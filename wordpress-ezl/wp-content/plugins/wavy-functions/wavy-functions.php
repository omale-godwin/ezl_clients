<?php
/**
 * Plugin Name: Wavy Theme Functions
 * Plugin URI: https://estudiopatagon.com
 * Description: This plugin enables core functions for <a href="https://estudiopatagon.com/projects/wavy-for-wordpress">Wavy - Modern & Lightweight Theme for WordPress</a> by <a href="https://estudiopatagon.com">EstudioPatagon</a>.
 * Version: 1.8.5
...
/* Check if Wavy Theme is activated */

if( get_template() != 'wavy' ){
    return;
}

if ( !class_exists('epcl_wavy_theme_functions') ) {
    ...
    if( !defined('EPCL_ITEM_ID') ) define('EPCL_ITEM_ID', '49866780'); // Do no change

    class epcl_wavy_theme_functions {
        ...
        public function load_csf_framework_widgets( ) {
            include(EPCL_PLUGIN_PATH.'/admin/framework/codestar-framework.php');
        } 
                ...
                if( is_archive() ){
                    ...
                    if( is_category() ){
                        ...
                        if( !empty($obj) ){
                            ...
                            if( !empty($term_meta) && isset($term_meta['archives_image']['id']) && $term_meta['archives_image']['id'] != '' ){
		...
		public function posts_columns($defaults){
            $defaults[EPCL_THEMEPREFIX.'_post_image'] = esc_html__('Image', 'wavy');
			return $defaults;
		}
        ...
        public function add_image_placeholders( $content ) {
            ...
            if( isset($epcl_theme['enable_lazyload_posts']) && $epcl_theme['enable_lazyload_posts'] === '1' ){
                $content = preg_replace_callback( '/<img .*?>/', function($matches) {
                    ...
                    $replaced = preg_replace( '/\bsrc\s*=\s*[\'"](.*?)[\'"]/', 'src="$1" loading="lazy"', $matches[0] );           
                    return $replaced;
                }, $content);
            }
            ...
            if( isset($epcl_theme['enable_lazyload_embed']) && $epcl_theme['enable_lazyload_embed'] === '1' ){
                $content = preg_replace_callback( '/<iframe .*?>/', function($matches) {
                    return preg_replace( '/\bsrc\s*=\s*[\'"](.*?)[\'"]/', 
                        'src="$1" loading="lazy"', $matches[0] ); } , $content);
            }      
